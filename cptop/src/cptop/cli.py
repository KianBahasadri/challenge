...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "scan":
        _print_scan(args.workspace, args.tags)
        return
    if args.command == "missing-tags":
        _print_missing_tags(args.workspace, args.tags)
        return

    CptopApp(args.workspace, args.tags).run()


def _print_scan(workspace: Path, tag_file: Path) -> None:
    stats = build_dashboard_stats(scan_workspace(workspace), workspace.resolve(), tag_file)
...
def _print_missing_tags(workspace: Path, tag_file: Path) -> None:
    stats = build_dashboard_stats(scan_workspace(workspace), workspace.resolve(), tag_file)
